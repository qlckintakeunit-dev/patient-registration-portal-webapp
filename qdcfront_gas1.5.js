...
function _bumpDataVersion() {
  ...
  // Best-effort push notify via Firebase Realtime Database REST API.
  // Fire-and-forget: if this fails or FIREBASE_DB_URL isn't configured,
  // clients simply rely on their fallback poll instead. muteHttpExceptions
  // guards against ever throwing out of this helper.
  if (FIREBASE_DB_URL && next !== null) {
    try {
      const base = FIREBASE_DB_URL.replace(/\/+$/, ''); // strip trailing slash(es) to avoid a double-slash path
      UrlFetchApp.fetch(base + '/dataVersion.json', {
        method: 'put',
...
function handleGetPatients(p) {
  ...
  try { cache.put(cacheKey, JSON.stringify(result), 30); } catch (e) { /* result too large for cache — non-fatal, just skip caching it */ }
  return result;
}
