...
// ═══════════════════════════════════════════════════════════════════
//  FIREBASE PUSH NOTIFY — optional, best-effort real-time nudge.
//  Fill in FIREBASE_DB_URL with your Realtime Database URL, e.g.
//    'https://your-project-id-default-rtdb.firebaseio.com'
//  (Firebase console → Realtime Database → copy the URL shown at the top).
//  Leave FIREBASE_DB_URL empty ('') to disable this — the app falls back
//  to plain polling automatically, nothing else breaks.
//
//  Recommended Realtime Database rules (scopes access to just this one
//  key, no auth/service-account setup needed since it only ever holds a
//  non-sensitive incrementing counter):
//    {
//      "rules": {
//        ".read": false,
//        ".write": false,
//        "dataVersion": { ".read": true, ".write": true }
//      }
//    }
// ═══════════════════════════════════════════════════════════════════
const FIREBASE_DB_URL = 'https://qdc-front-default-rtdb.asia-southeast1.firebasedatabase.app/';
...
function doGet(e) {
  ...
  try {
    ...
    if (action === 'getversion')   return json({ success: true, version: _getDataVersion() });
...
// ═══════════════════════════════════════════════════════════════════
//  DATA VERSION — cheap change-detection for near-real-time polling
//  Bumped by every handler that mutates DAILY REG SHEET / RECEIPT LOGS
//  (register/edit/delete/refund) and by onEdit for manual sheet edits.
//  Clients poll `getversion` (no sheet reads) and only refetch patient
//  data when the version actually changes.
// ═══════════════════════════════════════════════════════════════════
function _bumpDataVersion() {
  let next = null;
  try {
    const props = PropertiesService.getScriptProperties();
    next = (parseInt(props.getProperty('daily_data_version'), 10) || 0) + 1;
    props.setProperty('daily_data_version', String(next));
    CacheService.getScriptCache().put('daily_data_version', String(next), 21600);
  } catch (e) { /* non-fatal — worst case clients fall back to cache TTL */ }

  // NOTE: Firebase push-notify used to happen here via UrlFetchApp, but this
  // Apps Script project doesn't have the external_request OAuth scope granted
  // (confirmed via testFirebaseWrite — see below), so it always failed
  // silently. The browser now pushes the version bump directly to Firebase
  // right after a successful register/refund/delete/edit call — see
  // _fbBumpVersion() in index.html. This local version counter (via
  // PropertiesService/CacheService above) still works fine on its own as the
  // fallback-polling data source for the `getversion` action.
}

function _getDataVersion() {
  try {
    const cache = CacheService.getScriptCache();
    let v = cache.get('daily_data_version');
    if (v === null) {
      v = PropertiesService.getScriptProperties().getProperty('daily_data_version') || '0';
      cache.put('daily_data_version', v, 21600);
    }
    return v;
  } catch (e) { return '0'; }
}
...
// ═══════════════════════════════════════════════════════════════════
//  CHUNKED CACHE — CacheService caps each key at 100KB. handleGetPatients'
//  joined result can exceed that once DAILY REG SHEET grows large enough,
//  which silently disabled caching entirely (every request paid the full
//  read+join cost). These helpers split a large string across multiple
//  cache keys so caching keeps working past that limit.
// ═══════════════════════════════════════════════════════════════════
const _CACHE_CHUNK_SIZE = 90000; // stay safely under the 100KB/key cap
...
function _cacheGetChunked(cache, key) {
  try {
    ...
    for (let i = 0; i < count; i++) {
      ...
      if (part === null || part === undefined) return null; // a chunk expired/evicted — treat as a miss
      out += part;
    }
    return out;
  } catch (e) { return null; }
...
function _cachePutChunked(cache, key, str, ttlSeconds) {
  try {
    const count = Math.ceil(str.length / _CACHE_CHUNK_SIZE) || 1;
    const obj = { [key + ':meta']: String(count) };
    for (let i = 0; i < count; i++) {
      obj[key + ':' + i] = str.slice(i * _CACHE_CHUNK_SIZE, (i + 1) * _CACHE_CHUNK_SIZE);
    }
    cache.putAll(obj, ttlSeconds);
  } catch (e) { /* non-fatal — worst case this request just isn't cached */ }
}
...
function handleGetPrices() {
  ...
  if (testsSheet) {
    ...
    // Locate each price column by its header text instead of a fixed index,
    // so re-ordering/inserting columns in the sheet can't silently break pricing.
    const header = rows[0] || [];
    function findCol(matchers, fallbackIdx) {
      for (let i = 0; i < header.length; i++) {
        const h = String(header[i] || '').trim().toUpperCase();
        if (matchers.some(m => h === m || h.includes(m))) return i;
      }
      return fallbackIdx;
    }
    const COL_R      = findCol(['REGULAR PRICE', 'REGULAR'], 2);
    const COL_S      = findCol(['SENIOR PRICE', 'SENIOR'], 3);
    const COL_BSA    = findCol(['BSA PRICE', 'BSA'], 5);
    const COL_OM     = findCol(['QSD OM PRICE', 'OM PRICE', 'OM'], 6);
    const COL_YAKAP  = findCol(['YAKAP PRICE', 'YAKAP'], 7);
    const COL_AMANTE = findCol(['AMANTE PRICE', 'AMANTE'], 8);
    const COL_GOBSOL = findCol(['GOBSOL PRICE', 'GOBSOL'], 9);
    ...
    for (let i = 1; i < rows.length; i++) {
      ...
      // ── Price columns ─────────────────────────────────────────────
      const r      = parsePrice(rows[i][COL_R]);
      const s      = parsePrice(rows[i][COL_S]);
      const bsa    = parsePrice(rows[i][COL_BSA]);
      const om     = parsePrice(rows[i][COL_OM]);
      const yakap  = parsePrice(rows[i][COL_YAKAP]);
      const amante = parsePrice(rows[i][COL_AMANTE]);
      const gobsol = parsePrice(rows[i][COL_GOBSOL]);
      ...
      prices[testName.toUpperCase()] = { r, s, bsa, om, yakap, amante, gobsol };
    ...
    // Fallback: read from legacy PRICELIST tab (no BSA/OM/YAKAP/AMANTE/GOBSOL columns there)
...
function handleRefundPatient(data) {
  ...
  // Essential value writes only here — cosmetic dropdown/font styling is
  // deferred below (after _bumpDataVersion) so the Firebase push isn't
  // held up behind extra getRange()/setX() round-trips.
  let dailyUpdated = false;
  let dailySheetRow = null;
  const daily = ss.getSheetByName(TAB.DAILY);
  if (daily) {
  ...
  // Data is committed — notify Firebase/clients now, before the cosmetic
  // styling below.
  _bumpDataVersion();

  // Cosmetic formatting — runs after the push, doesn't delay it.
  if (dailySheetRow) {
    _setupDailyDropdowns(daily, dailySheetRow);
    daily.getRange(dailySheetRow, COL.STATUS).setFontColor('#757575')
                                              .setFontWeight('bold');
  }
  if (logSheetRow) {
    logSheet.getRange(logSheetRow, RLOG.STATUS).setFontColor('#757575')
                                                .setFontWeight('bold');
  }
...
function handleEditPatient(data) {
  ...
  const payType   = (data.paymentType   || 'CASH').toString().trim().toUpperCase().replace('G-CASH','GCASH');
  ...
  if (logSheet) {
    ...
    if (lastRow >= 2) {
      ...
      for (let i = numVals.length - 1; i >= 0; i--) {
        ...
        logSheet.getRange(sheetRow, 1, 1, 29).setValues([rowData]);
...
function handleRegister(data) {
  // ── Idempotency check ────────────────────────────────────────────────
  // The client sends a stable clientTxnId per registration attempt (same
  // id is resent if the user retries Confirm & Save after a timeout/dropped
  // connection). If we've already processed this exact attempt — e.g. the
  // row was saved but the success response never made it back to the
  // client over a bad connection — return the original result instead of
  // creating a second (duplicate) patient record.
  const clientTxnId = String(data.clientTxnId || '').trim();
  const cache = CacheService.getScriptCache();
  if (clientTxnId) {
    const cached = cache.get('reg_' + clientTxnId);
  ...
  // ── Lock around number assignment + row append ──────────────────────
  // Prevents two near-simultaneous registrations (e.g. two front-desk
  // devices saving at the same moment) from reading the same "last number"
  // and appending rows with a racy/duplicate patient number.
  const lock = LockService.getScriptLock();
  lock.waitLock(30000);

  let nextNum, paddedNum;
  try {
    const lastRow = sheet.getLastRow();
    nextNum = 1;
    if (lastRow >= 2) {
    ...
    paddedNum = String(nextNum).padStart(4, '0');

    var dailyFmt = _handleRegisterAppendRow(sheet, data, paddedNum);
  } finally {
    lock.releaseLock();
  }
  ...
  var logFmt = null;
  try {
    logFmt = _writeReceiptLog(ss, data, nextNum, paddedNum,
                     parseFloat(data.labSubtotal)     || 0,
                     parseFloat(data.checkupSubtotal) || 0,
                     parseFloat(data.cadSubtotal)     || 0,
                     parseFloat(data.kitsSubtotal)    || 0,
                     parseFloat(data.xraySubtotal)    || 0,
                     parseFloat(data.drugSubtotal)    || 0,
                     parseFloat(data.utzSubtotal)     || 0,
                     parseFloat(data.discountAmt)     || 0,
                     _handleRegisterFinalAmount(data),
  ...
  const result = { success: true, number: nextNum };
  // Cache the result for 6 hours (max allowed) so a retry with the same
  // clientTxnId short-circuits above instead of writing another row.
  if (clientTxnId) {
    cache.put('reg_' + clientTxnId, JSON.stringify(result), 21600);
  }

  // Data is committed at this point — notify Firebase/clients now instead
  // of after the cosmetic formatting below. This is the main latency fix:
  // it's what makes the update actually show up in ~1-2s instead of 6-7s.
  _bumpDataVersion();

  // Cosmetic formatting — runs after the push, doesn't delay it.
  if (dailyFmt) {
    _formatDailyRow(sheet, dailyFmt.newRow, dailyFmt.paymentStatus, dailyFmt.paymentType,
                    dailyFmt.disAmt, dailyFmt.xraySubtotal, dailyFmt.drugSubtotal, dailyFmt.utzSubtotal);
    _setupDailyDropdowns(sheet, dailyFmt.newRow);
  }
  if (logFmt) {
    _formatReceiptLogRow(logFmt.logSheet, logFmt.newRow, logFmt.payStatus, logFmt.payType,
                         logFmt.disAmt, logFmt.labSub, logFmt.checkupSub, logFmt.cadSub,
                         logFmt.kitsSub, logFmt.xraySub, logFmt.drugSub, logFmt.utzSub);
  }

  return result;
}

// Total amount, computed the same way handleRegister always has.
function _handleRegisterFinalAmount(data) {
...
// Builds the DAILY row + formats it. Extracted unchanged from the original
// handleRegister body so it can run inside the LockService critical section.
function _handleRegisterAppendRow(sheet, data, paddedNum) {
  const labSubtotal      = parseFloat(data.labSubtotal)      || 0;
  const checkupSubtotal  = parseFloat(data.checkupSubtotal)  || 0;
  const cadSubtotal      = parseFloat(data.cadSubtotal)      || 0;
  const kitsSubtotal     = parseFloat(data.kitsSubtotal)     || 0;
  ...
  sheet.appendRow(row);
  const newRow = sheet.getLastRow();
  // Formatting/dropdowns are cosmetic and don't affect what the client
  // reads back, so they're deferred to run AFTER _bumpDataVersion() —
  // see handleRegister — instead of blocking the Firebase push behind
  // a dozen extra getRange()/setX() round-trips.
  return {
    newRow, paymentStatus, paymentType, disAmt,
...
function _writeReceiptLog(ss, data, num, paddedNum,
  ...
  // Deferred for the same reason as _handleRegisterAppendRow above —
  // see handleRegister, formatting now runs after _bumpDataVersion().
  return {
    logSheet, newRow, payStatus, payType, disAmt,
    labSub, checkupSub, cadSub, kitsSub, xraySub, drugSub, utzSub,
  };
}
...
function handleGetPatients(p) {
  // ── Response cache ──────────────────────────────────────────────
  // Keyed by data version + the exact query params. As long as nothing
  // has changed (version unchanged), repeated/concurrent requests for the
  // same page+filters (by the same or different cashiers) are served from
  // cache instead of re-reading & re-joining both sheets. Short TTL is a
  // safety net in case a write path is ever missed.
  const version  = _getDataVersion();
  const cacheKey = 'gp_' + version + '_' + [
    p.page || 1, p.search || '', p.status || '', p.referred || '',
    p.dateFrom || '', p.dateTo || '',
  ].join('|').slice(0, 200);
  const cache = CacheService.getScriptCache();
  try {
  ...
  const result = { success: true, records: slice, total, page, pages };
  _cachePutChunked(cache, cacheKey, JSON.stringify(result), 30);
  return result;
}
...
// ═══════════════════════════════════════════════════════════════════
//  testFirebaseWrite — TEMP DIAGNOSTIC, run manually from the editor.
//  Select "testFirebaseWrite" from the function dropdown at the top of
//  the Apps Script editor, click Run, then click "Execution log" at the
//  bottom — no need to visit the Executions page at all.
// ═══════════════════════════════════════════════════════════════════
function testFirebaseWrite() {
  Logger.log('FIREBASE_DB_URL = ' + FIREBASE_DB_URL);

  if (!FIREBASE_DB_URL) {
    Logger.log('FIREBASE_DB_URL is empty — nothing to test.');
    return;
  }

  const base = FIREBASE_DB_URL.replace(/\/+$/, '');
  const url  = base + '/dataVersion.json';
  Logger.log('PUT -> ' + url);
